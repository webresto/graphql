...
export default {
  Query: {
    menuContext: {
      ...
      fn: async (
        parent: any,
        args: { orderId?: string; cookingPointId?: string; lat?: number; lon?: number },
      ) => {
        try {
          // The order is looked up rather than taken on trust: its kitchen and
          // its `maxWaitMinutes` are the whole reason to pass an id, and a client
          // cannot be the source of them — that is what would let a basket be
          // priced at a kitchen nobody assigned to it.
