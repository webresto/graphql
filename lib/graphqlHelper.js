 ...
 */
async function dropStoppedDishes(modelname, records, orderId) {
    if (modelname !== "dish" || !Array.isArray(records) || !records.length)
        return records;
...
function sanitizeCriteria(modelname, criteria) {
    ...
    switch (modelname) {
        case 'dish':
            // Stock belongs to the pair "product + cooking point" and is not a column
            // of this model any more, so stopped products are dropped from the result
            // by dropStoppedDishes() instead of being excluded by the query.
            where.isDeleted = false;
            break;
        case 'group':
            where.isDeleted = false;
            ...
            switch (modelRelationType) {
                ...
                case "collection":
                    resolvers[key] = async (parent, args, context) => {
                        ...
                        let result = null;
                        let relationCriteria = sanitizeCriteria(modelAttribute[modelRelationType], {});
                        if (modelAttribute.through && sails.models[modelAttribute.through] && sails.models[modelAttribute.through].attributes.sortOrder) {
                            relationCriteria.sort = 'sortOrder ASC';
                        }
                        result = (await sails.models[modelname].findOne({ id: parent.id }).populate(key, relationCriteria))[key];
